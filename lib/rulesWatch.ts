 ...
 * cuando la semana anterior ya terminó, calcula para el PERFIL ÚNICO sus
...
export async function watchWeeklyRules(now = Date.now()): Promise<RulesWatchResult> {
  ...
  const target = getPrimaryProfile();
  if (!target) {
  ...
  for (const list of byDay.values()) {
    ...
    violationLosses += day.violationLosses;
  }

  const findings: string[] = [];
  if (cutIgnored > 0) findings.push(`${cutIgnored} corte(s) ignorado(s)`);
  if (violations > 0) findings.push(`${violations} fuera de pool (${violationLosses} D)${banned ? ` · ${banned} prohibidos` : ''}`);
  ...
  const res = await sendPush({
    ...
    body: `${record} en ${matches.length}p · ${findings.join(' · ')}`,
    url: '/reglas',
  });
