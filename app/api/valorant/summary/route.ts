...
export async function GET(req: NextRequest) {
  ...
  if (access === 'forbidden') {
    return Response.json({ error: 'Ese perfil no es público', code: 'FORBIDDEN' }, { status: 403 });
  }
  ...
  try {
    // Compat: refresh=1 en GET = revalidación síncrona (cooldown compartido).
    if (sp.get('refresh') === '1') {
    ...
    const summary = await getValSummary({ days, maxFetch: limit, playerId, viewer, accountName, accountTag });
    return Response.json(summary, { headers: { 'Cache-Control': 'no-store' } });
  } catch (err) {
    const code = (err as { code?: string })?.code ?? 'HTTP';
    const isKeyIssue = code === 'KEY_MISSING' || code === 'KEY_EXPIRED' || code === 'FORBIDDEN';
    return Response.json(
